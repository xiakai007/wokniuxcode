layui.extend({
    ...
    self.preventRepeatClick = function (elem1,elem2) {
        // $(elem1).find(elem2).trigger('click');
        // 单击之后提交按钮不可选,防止重复提交
        let DISABLED = 'layui-btn-disabled';
        // 增加样式
        $(elem1).find(elem2).addClass(DISABLED);
        // 增加属性
        $(elem1).find(elem2).attr('disabled', 'disabled');
        setTimeout(function () {
            $(elem1).find(elem2).removeClass(DISABLED).attr('disabled', false);
        }, 3000);
    }
    //初始化整个页面
