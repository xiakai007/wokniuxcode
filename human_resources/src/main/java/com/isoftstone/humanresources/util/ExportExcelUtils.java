...
import com.isoftstone.humanresources.domain.workTime.ExportFileModel;
...
public class ExportExcelUtils {
     ...
     * @param <T>
     */
    public static <T> void exportManySheet(String excelName ,List<ExportFileModel> fileModels, HttpServletResponse response) {
        ...
        try {
            ...
            //创建一个居中格式
            style.setAlignment(HSSFCellStyle.ALIGN_CENTER);
            for(ExportFileModel fileModel :fileModels) {
                //在Workbook中，创建一个sheet，对应Excel中的工作薄（sheet）
                HSSFSheet sheet = wb.createSheet(fileModel.getExcelName());
                // 填充工作表
                fillSheet(sheet, fileModel.getList(), fileModel.getFieldMap(), style);
            }
            //将文件输出
            OutputStream ouputStream = response.getOutputStream();
            wb.write(ouputStream);
